...
class alipayfull_link {
    ...
    public function get_paylink($params){
        if (!function_exists("openssl_open")){
            return '<span style="color:red">Fatal Error: OpenSSL support is missing.</span>';
        }
        if (!function_exists("scandir")){
            return '<span style="color:red">Fatal Error: PHP function scandir is required. Edit the disable_function in php.ini to solve the problem.</span>';
        }
    ...
    public function normal_f2fpay($params){
        ...
        switch ($qrPayResult->getTradeStatus()){
            case "SUCCESS":
                ...
                return $skin_raw;
            case "FAILED":
                return "QR code failed to generate.";
            case "UNKNOWN":
                return "Internal error: Unknown status of invoice.";
            default:
